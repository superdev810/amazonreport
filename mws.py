...
class MWS(object):
    ...
    def make_request_with_data_in_body(self, extra_data, method="GET", **kwargs):
        """Make request to Amazon MWS API with these parameters
        """
        ...
        request_description = '&'.join(['%s=%s' % (k, urllib.quote(params[k], safe='-_.~').encode('utf-8')) for k in sorted(params)])
        signature = self.calc_signature(method, request_description)
        url = '%s%s?&Signature=%s' % (self.domain, self.uri, urllib.quote(signature))

        #url = '%s%s' % (self.domain, self.uri)
        headers = {'User-Agent': 'python-amazon-mws/0.0.1 (Language=Python)'}
        headers.update(kwargs.get('extra_headers', {}))
    ...
    def tree2dict(self, root_key, param_dict):
        params = {}
        if dict is not None:
            prefix = ''
            if root_key != '':
            ...
            for key, value in param_dict.iteritems():
                target_key = '%s%s' % (prefix, key)
                if isinstance(value, dict):
                    params.update(self.tree2dict(target_key, value))
                elif isinstance(value, list):
                    params.update(self.enumerate_tree2dict(target_key, value))
                else:
                    params[target_key] = value
        return params

    def enumerate_tree2dict(self, param, values):
        """
...
class InboundShipments(MWS):
    ...
    def create_inbound_shipment_plan(self, ship_from_address, inbound_shipment_plan_request_items,
                                     ship_to_country_code=None, ship_to_country_subvision_code=None,
        ...
        data = dict(Action='CreateInboundShipmentPlan',
                    ...
                    ShipToCountrySubdivisionCode=ship_to_country_subvision_code,
                    LabelPrepPreference=label_prep_preference,
                    )
        data.update(
            self.enumerate_param('InboundShipmentPlanRequestItems.member.', inbound_shipment_plan_request_items))
        data = self.tree2dict('',data)
        print data
        return self.make_request_with_data_in_body(data, "POST")

    def create_inbound_shipment(self, shipmentid, inbound_shipment_header, inbound_shipment_items):
        """ Returns information on available inventory """
        ...
        data.update(
            self.enumerate_param('InboundShipmentItems.member.', inbound_shipment_items))
        data = self.tree2dict('', data)
        print 'CreateInboundShipment'
        print data
        return self.make_request_with_data_in_body(data, "POST")

    def update_inbound_shipment(self, shipmentid, inbound_shipment_header, inbound_shipment_items):
        """ Returns information on available inventory """
